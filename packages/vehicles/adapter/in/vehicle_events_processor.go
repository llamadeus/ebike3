...
func MakeVehicleEventsProcessor(vehicleService in.VehicleService) *micro.EventsProcessor {
	return micro.NewEventsProcessor(micro.HandlersMap{
		events.VehiclesVehicleCreatedEventType: micro.NewEventHandler(func(payload events.VehicleCreatedEvent) error {
			...
			return vehicleService.CreateVehicleView(id, type_, payload.PositionX, payload.PositionY, payload.Battery)
		}),
		events.VehiclesVehicleUpdatedEventType: micro.NewEventHandler(func(payload events.VehicleUpdatedEvent) error {
			slog.Info(
				"vehicle updated",
				"id", payload.ID,
				"positionX", payload.PositionX,
